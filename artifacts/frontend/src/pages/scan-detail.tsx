      ...
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <div className="flex gap-2 shrink-0 flex-wrap justify-end">
          ...
          <Button onClick={generate} disabled={isStreaming || disabled} size="sm"
            ...
            {loading ? <><Loader2 className="w-4 h-4 animate-spin" />Generatingâ€¦</> : <>{icon}{buttonLabel}</>}
          </Button>
        </div>
      </CardHeader>
      <CardContent>
        ...
        {done && (
          <div className="mt-4 pt-3 border-t border-border space-y-2">
            <p className="text-xs text-muted-foreground">AI Security Engine Â· Ask a follow-up below</p>
...
export default function ScanDetail() {
  ...
  function startSSE() {
    ...
    es.addEventListener("engine", (e: MessageEvent) => {
      try {
        ...
        if (d.status === "scanning") {
          ...
          appendLog("engine", `â–¶ ${d.engine} â€” ${d.message ?? "scanning"}`);
        } else {
          setActiveEngine(null);
          appendLog("engine", `âœ“ ${d.engine} â€” ${d.message ?? "done"}`);
        }
      } catch { /* */ }
    });
    ...
    es.addEventListener("complete", (e: MessageEvent) => {
      try {
        const d = JSON.parse(e.data);
        appendLog("log", `âœ… ${d.message ?? "Scan complete"}`);
      } catch { /* */ }
      setSseActive(false);
      setActiveEngine(null);
      es.close();
    ...
    es.addEventListener("error", (e: MessageEvent) => {
      try {
        const d = JSON.parse(e.data);
        appendLog("error", `âŒ Error: ${d.message ?? "Scan failed"}`);
      } catch { /* */ }
      setSseActive(false);
      setActiveEngine(null);
      es.close();
  ...
  function exportPDF() {
    ...
    const doc = new jsPDF();
    doc.setFontSize(20);
    doc.text("Bug Finder Pro â€” Scan Report", 14, 22);
    doc.setFontSize(10);
        ...
        <div className="flex items-center space-x-2 flex-wrap gap-y-2">
          {scan.status === "running" && (
            <Button variant="outline" className="border-yellow-500/40 text-yellow-400 hover:bg-yellow-500/10"
              ...
              {pauseScan.isPending ? "Pausingâ€¦" : "Pause"}
            </Button>
          )}
          {scan.status === "paused" && (
            <Button variant="outline" className="border-green-500/40 text-green-400 hover:bg-green-500/10"
              ...
              {resumeScan.isPending ? "Resumingâ€¦" : "Resume"}
            </Button>
          )}
          {isRunning && (
            <Button variant="outline" className="border-orange-500/40 text-orange-400 hover:bg-orange-500/10"
              ...
              {cancelScan.isPending ? "Cancellingâ€¦" : "Stop Scan"}
            </Button>
          )}
          ...
          <Button variant="outline" onClick={startSSE} disabled={sseActive}>
            ...
            {sseActive ? "Streamingâ€¦" : "Live Stream"}
          </Button>
        </div>
      </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-blue-500 flex items-center gap-2">
                ...
                {activeEngine ? `Running: ${activeEngine}` : "Scan in progressâ€¦"}
              </span>
            ...
            <p className="text-xs text-blue-400/60 mt-2">
              {scan.findings_count > 0
                ...
                : "Scanning target for vulnerabilitiesâ€¦"}
            </p>
          </CardContent>
        </Card>
      )}
          ...
          <CardContent className="px-4 pb-4">
            <div className={`font-mono text-xs rounded p-3 max-h-64 overflow-y-auto leading-relaxed space-y-0.5 ${terminalDark ? "bg-black/80" : "bg-slate-100 dark:bg-slate-800"}`}>
              {sseActive && sseLog.length === 0 && (
                <span className="text-violet-400/70 animate-pulse">Connecting to scannerâ€¦</span>
              )}
              {sseLog.map((line, i) => (
                <div key={i} className={
      ...
      <Tabs defaultValue="findings" className="w-full">
        ...
        <TabsContent value="diff" className="pt-6">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                <CardTitle>Scan Diff â€” Delta View</CardTitle>
              ...
              {!diffLoaded && (
                <Button size="sm" variant="outline" onClick={loadDiff} disabled={diffLoading}>
                  {diffLoading ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Loadingâ€¦</> : <><GitCompare className="w-4 h-4 mr-2" />Load Diff</>}
                </Button>
              )}
            </CardHeader>
            <CardContent>
        ...
        <TabsContent value="kill-chains" className="pt-6">
          <Card>
            ...
            <CardContent>
              {killChainsLoading ? (
                <div className="py-8 text-center text-muted-foreground text-sm">Analyzing findingsâ€¦</div>
              ...
              <div className="flex gap-3 mb-4">
                <Select value={compareTargetId} onValueChange={setCompareTargetId}>
                  <SelectTrigger className="flex-1">
                    <SelectValue placeholder="Select a scan to compare againstâ€¦" />
                  </SelectTrigger>
                  <SelectContent>
                    {otherScans.length === 0 ? (
                      ...
                      <SelectItem key={s.id} value={s.id}>
                        {s.target_url.replace(/https?:\/\//, "").slice(0, 35)} â€” Risk: {s.risk_score}/100 â€” {format(new Date(s.created_at), "MMM d")}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <Button size="sm" variant="outline" onClick={runDeduplicate} disabled={dedupeLoading || isRunning}>
                {dedupeLoading ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Analyzingâ€¦</> : <>Detect Duplicates</>}
              </Button>
            </CardHeader>
            <CardContent>
              ...
              {dedupeResult && (
                dedupeResult.duplicates.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    <p className="text-xs text-muted-foreground">{dedupeResult.duplicates.length} duplicate group(s) found â€” consider merging or dismissing lower-quality duplicates</p>
